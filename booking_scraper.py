    ...
    def _parse_html(self, soup, city, checkin, checkout, nights):
        ...
        for card in cards[:15]:
            try:
                ...
                # Simple Price Parsing: Just find all Euro values and take the one that makes sense
                price_matches = re.findall(r'€\s*([\d\.,\s]+)', card_text)
                valid_prices = []
                for p in price_matches:
                    digits = "".join(re.findall(r'\d+', p))
                    if digits:
                        val = int(digits)
                        if val > 10: valid_prices.append(val)
